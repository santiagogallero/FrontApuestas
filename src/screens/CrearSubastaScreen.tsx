...
export function CrearSubastaScreen({ onNavigate }: Props) {
  ...
  const crear = async () => {
    ...
    if (!ubicacion.trim()) { Alert.alert('Falta ubicación', 'Ingresá la ubicación de la subasta.'); return; }
    ...
    try {
      ...
      const res = await apiCrearSubasta({ ubicacion: ubicacion.trim(), categoria, items });
      Alert.alert(
        'Subasta creada',
